...
- V117: error INSPECTION goes through the `errors` library, NEVER a type assertion/switch on an error value. classify a wrapped error via `errors.As(err,&target)` (typed) + `errors.Is(err,sentinel)` (sentinel), ⊥ `err.(*T)` / `err.(I)` / `switch err.(type)` — a bare assertion MISSES a wrapped error (`fmt.Errorf("…: %w", e)`) ⇒ silent misclassify (e.g. a `*ServerClosedError` wrapped by Login slips past `err.(*packet.ServerClosedError)`). applies to ALL shipped pkgs (`packer`/`packet`/`net6`/`net7`/`network`/`client`/`master`/`physics`/`record/*`) + `*_test.go`; gitignored `cmd/` exempt. KNOWN violation: `client/client.go:355` `err.(*packet.ServerClosedError)` (T145). idiom invariant (cf. V69/V71/V97); behavior-PRESERVING except it now also catches the wrapped form (which is the point). enforced by review (`go vet`/staticcheck won't flag it) — grep `\.\(\*?.*[Ee]rr` in shipped code = empty.
...
shared snapshot repr (V112–V115): T132–T140 + T122/T123/T124/T125/T136/T137/T141 = `x` (done); T126/T127 superseded `x`; T88–T93 teehistorian perf `x`. e2e harness validated against real DDNet sixup + vanilla teeworlds 0.7 (T137, in-network). PENDING = ONLY T142–T144 (V116): multi-stage SOURCE Docker builds of both servers with debug enabled (`dbg_dummies`) → genuinely multi-character snapshots, then strengthen parity to `chars > 1`. everything else `x`.
