...
T69|.|nil/empty structural-input safety (V70): physics.NewCollision(nil) + physics.NewCore(nil,…) + client.NewMapView(nil)/MapView() handle a nil/empty *twmap.Map as an empty all-solid world (⊥ deref m.GameLayers()); + tests passing nil map → no panic, OOB = ClassSolid/TileSolid|V70,I.robust
T70|.|input-validation sweep (V70): audit every exported func/method/ctor/Option across packet/packer/network/net6/net7/physics/client/master for unguarded caller input; clamp config (V62) / return error / guard nils per policy; document each contract in godoc; + per-package hostile-input tests (nil, "", negative, oversized, truncated/garbage wire bytes, bad version/address) asserting NO panic + sane error/clamp|V70,V62,I.robust
