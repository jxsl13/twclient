...
- V91: **round-trip fixpoint, BYTE-EQUAL** — `Parse(x).WriteTo()` reproduces `x` BYTE-FOR-BYTE on every fixture (`testdata/Tutorial.demo`, `Tutorial.client.demo`, `Tutorial.gho`), and `Parse(WriteTo(Parse(x)))` yields Header+Chunks EQUAL to `Parse(x)`. byte-equality is the HARD target (user requirement), ⊥ merely semantic equality — the huffman + intpack + delta re-encode must match DDNet's canonical output exactly; any unavoidable divergence is documented + justified in §B. demo + ghost each verified independently.
