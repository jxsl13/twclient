...
T88|.|MEASURE-FIRST: add `BenchmarkParse` (+ `-benchmem`) on a large testdata fixture (e.g. the 21 KB / 287k-record one or a recorded 1.8 MB file); record baseline allocs/op, B/op, ns/op. gate — ⊥ optimize before this|V85,V49
T89|.|no-copy body parse (V84a): drop `packer.NewUnpacker`'s make+copy of the whole body; parse in place over the source slice (`Unpacker.ResetView`/cursor). re-bench vs T88|V84
T90|.|pre-allocate + direct reads (V84b,c): size `Records` from `len(body)/avgRecordBytes` estimate; remove the `ints()` per-record temp slice — decode fields straight into each record struct. re-bench|V84
T91|.|alias variable-length fields into retained source (V84d): `File` holds the source bytes; `Message.Msg`/`Ex.Data`/`Drop`/`ConsoleCommand` strings sub-slice it instead of copying; document File-lifetime ownership. round-trip still byte-identical (V77)|V84,V77
T92|.|re-bench + verify (V85): allocs/op dropped materially vs T88 baseline; round-trip byte-identical on all fixtures; full teehistorian suite green incl Examples|V85,V77
T93|.|IF T92 profile shows `Record` interface boxing still dominates → value/tagged representation (concrete `Record` struct + `Kind Marker`, 0 per-record alloc); re-bench. measure-first — skip if boxing ⊥ the bottleneck|V85
...
demo+ghost record readers (byte-equal de/serialize, whole+streaming): T98–T107 = `.` (pending). build order per format: header → chunk codec → whole-file byte-equal → streaming → hostile/doc. demo (T99–T102) ∥ ghost (T103–T106) independent; T98 skeleton first, T107 last. foundation pkg packer+huffman/v2 only (V90); BYTE-EQUAL round-trip is the hard target (V91), fixtures Tutorial.demo/.client.demo/.gho.
teehistorian perf (alloc reduction, measure-first): T88–T93 = `.` (pending). build order: T88 (bench baseline) → T89 (no-copy body) → T90 (preallocate+direct reads) → T91 (alias sub-fields) → T92 (re-bench/verify) → T93 (value Record only if boxing dominates). V49/V85 gate — T88 BEFORE T89–T91; round-trip byte-identical preserved (V77).
teehistorian (read+write-back, for ML behavior-cloning): T80–T87 = `.` (pending). build order: T80 (doc/pin) → T81 (header) → T82 (body codec) → T83 (implicit tick) → T84 (delta↔abs) → T85 (EX registry) → T86 (round-trip fixpoint) → T87 (Ticks ML helper). foundation pkg, packer+packet only (V76); round-trip semantic-equal w/ byte-identical target (V77).
teehistorian relocation + streaming: T94–T97 = `.` (pending). build order: T94 (move → `record/teehistorian`, update importers) → T95 (streaming Decoder) → T96 (streaming Encoder) → T97 (stream≡nonstream + byte-identical cross-check). V87 (location under `record`), V88 (parallel streaming impl, O(1) memory, ⊥ ReadAll), V89 (stream≡Parse + byte-identical). NOTE the streaming↔in-memory ownership difference (V88 copies vs V84 aliases).
