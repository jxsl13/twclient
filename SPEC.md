...
parse-ownership alignment (V60): T51–T52 = `.` (pending). build order: T51 (ServerInfo/PlayerInfo → packet; net6/net7 ParseInfoResponse) → T52 (master consumes parsers, delete master-side decode). answers "does parsing belong in master?" → NO; version-specific parse ⊂ net6/net7, result struct ⊂ packet. behavior unchanged (V48).
