...
B6|2026-06-13|OPEN: net6/net7 Login has NO vital/ctrl RETRANSMISSION. The background reader/resender starts only AFTER Login (client/client.go), so during handshake+login nothing resends a lost packet. A single dropped CONNECT, INFO, READY, or the server's MAP_CHANGE/CON_READY/SV_READYTOENTER is never resent → the recv-loop (Handshake/recvUntilMapChange/recvUntilConReady/recvUntilReadyToEnter in net6/session.go + net7/session.go) hits the read deadline and aborts: `session06: recv waiting for map_change: read udp …: i/o timeout`. Surfaces under packet loss — many concurrent clients from one host (UDP buffer overflow) or any lossy real network. DDNet `CNetConnection::Update` resends unacked vitals + re-sends CONNECT on a timer; we don't.|PLANNED (T59, V68): on recv-timeout during Login, resend the pending vital (INFO/READY) and re-send CONNECT in Handshake, bounded retries, before aborting — mirror CNetConnection::Update. Meanwhile mitigated (NOT cured) by paced connects (cmd/racebot flood gate); paced connect does not help real-network loss.
```
