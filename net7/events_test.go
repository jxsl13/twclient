...
func TestObjAsMessage07(t *testing.T) {
	...
	ci = append(ci, packer.PackStr("nameless")...)
	ci = append(ci, packer.PackStr("clan")...)
	ci = append(ci, packInts(-1)...) // country
	for i := 0; i < 6; i++ {
		ci = append(ci, packer.PackStr("part")...)
	}
	...
	s.processClientDrop(append(packInts(5), append(packer.PackStr("timeout"), packInts(0)...)...))
...
func TestVoteAndKill07(t *testing.T) {
	...
	vs := append(packInts(-1, 0, 25), append(packer.PackStr("kick x"), packer.PackStr("afk")...)...)
	s.processVoteSet(vs)
